...
// ─── Palette tokens (not Tailwind arbitrary — used in inline styles where needed) ───
// ── Brand anchors
const ESPRESSO   = '#4D310A'
const BRONZE     = '#7D5B24'

// ── Dark backgrounds
const DEEP       = '#2B1B0C'   // deepest dark (was OBSIDIAN)
const OBSIDIAN   = '#2B1B0C'   // alias kept for existing refs
const OBSIDIAN_MID = '#4D310A' // alternate dark (was near-black, now espresso)

// ── Gold scale
const GOLD         = '#C6A261'  // main interactive gold (was #B8973A)
const GOLD_LIGHT   = '#E2C98F'  // hover highlight
const GOLD_DIM     = 'rgba(198,162,97,0.28)'  // borders/dividers
const GOLD_SOFT    = '#C6A261'
const GOLD_BRAND   = '#7D5B24'
const GOLD_CHAMPAGNE = '#F0DEB8'

// ── Text / surface
const CREAM      = '#FCF9F4'   // primary text on dark (was #F0E6CC)
const CREAM_WARM = '#E7DCCB'   // secondary text on dark
const CHAMPAGNE  = '#CDBA9F'   // muted text / subtle borders

// ── Light section palette
const IVORY      = '#F4EEE5'   // light section bg
const PORCELAIN  = '#FCF9F4'   // elevated light surface
const ESPRESSO_TEXT = '#4D310A' // primary text on light
const BROWN_TEXT    = '#72563A' // secondary text on light
const TAUPE         = '#A78D6E' // muted text on light
const BORDER_LIGHT  = '#CDBA9F' // borders on light sections
...
const GALA_PHOTOS: Record<string, { src: string; caption: string }[]> = {
  '2024': [
    {
      src: 'https://images.unsplash.com/photo-1764255510960-deee566a91f0?w=1000&h=700&fit=crop&auto=format',
      caption: '2024 Gala — Evening Dining',
    },
    {
      src: 'https://images.unsplash.com/photo-1764269719300-7094d6c00533?w=900&h=600&fit=crop&auto=format',
      caption: '2024 Gala — Masquerade',
    },
    {
      src: 'https://images.unsplash.com/photo-1768508950243-16fd93e88d7f?w=900&h=600&fit=crop&auto=format',
      caption: '2024 Gala — VIP Dinner Table',
    },
    {
      src: 'https://images.unsplash.com/photo-1768508950778-9ba70d4445e9?w=900&h=600&fit=crop&auto=format',
      caption: '2024 Gala — Black-Tie Reception',
    },
  ],
  '2025': [
    {
      src: 'https://images.unsplash.com/photo-1712315071307-bb144277ba8c?w=1000&h=700&fit=crop&auto=format',
      caption: '2025 Gala — Grand Opening',
    },
    {
      src: 'https://images.unsplash.com/photo-1776142750401-1e894d4384fa?w=900&h=600&fit=crop&auto=format',
      caption: '2025 Gala — Formal Evening',
    },
    {
      src: 'https://images.unsplash.com/photo-1764255510960-deee566a91f0?w=900&h=600&fit=crop&auto=format&crop=bottom',
      caption: '2025 Gala — Chairman Welcome',
    },
    {
      src: 'https://images.unsplash.com/photo-1764269719300-7094d6c00533?w=900&h=600&fit=crop&auto=format&crop=top',
      caption: '2025 Gala — Dance Floor',
    },
  ],
}

// 9 slots × 2 images = 18 total. Every ZTB photo used exactly once.
// Grid: 6 columns, 3 rows. Landscape/wide images paired to wide slots; verticals to narrow.
// 9 slots × 2 = 18 unique ZTB photos — no image repeated.
// Row 1: LW1↔LW1b · LL1↔L7 · V1↔V2
// Row 2: V3↔V4    · L2↔L6  · L4↔L1
// Row 3: L8↔L5    · V5↔V6  · LW2↔LW2b
const MOSAIC: { pair: [string, string]; delay: number; col: string; row: string }[] = [
  // Row 1
  { pair: [ztbLW1,  ztbLW1b], delay: 0.0, col: '1 / 4', row: '1 / 2' }, // wide: sunset team, two angles
  { pair: [ztbLL1,  ztbL7  ], delay: 1.8, col: '4 / 6', row: '1 / 2' }, // medium: 2024 champions + belt celebration
  { pair: [ztbV1,   ztbV2  ], delay: 3.5, col: '6 / 7', row: '1 / 2' }, // narrow: celebration + dugout smiles
  // Row 2
  { pair: [ztbV3,   ztbV4  ], delay: 2.2, col: '1 / 2', row: '2 / 3' }, // narrow: fielder + glove portrait
  { pair: [ztbL2,   ztbL6  ], delay: 0.8, col: '2 / 5', row: '2 / 3' }, // wide: pitcher + batter hitting
  { pair: [ztbL4,   ztbL1  ], delay: 4.2, col: '5 / 7', row: '2 / 3' }, // medium: batter at plate + base handshake
  // Row 3
  { pair: [ztbL8,   ztbL5  ], delay: 1.5, col: '1 / 3', row: '3 / 4' }, // medium: team handshake + coaching circle
  { pair: [ztbV5,   ztbV6  ], delay: 3.8, col: '3 / 4', row: '3 / 4' }, // narrow: batter swing + coach huddle
  { pair: [ztbLW2,  ztbLW2b], delay: 5.5, col: '4 / 7', row: '3 / 4' }, // wide: team walk, two angles
]
...
const LUX_CARD_BORDER = 'rgba(198,162,97,0.14)'
...
function MosaicPlaceholder({ label }: { label?: string }) {
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, #111009 0%, #1c180e 50%, #0f0d09 100%)',
      }}
    >
      <div
...
function CarouselSlot({
  ...
  return (
    ...
    >
      {/* Use index as key — list is always exactly length 2 and never reorders */}
      {pair.map((src, i) => (
        <div
          key={i}
      ...
      {paused && (
        <div className="absolute bottom-2 left-2 z-10">
          <span
            className="text-white/60 text-[9px] tracking-widest uppercase px-2 py-1"
...
// ─── VideoCarouselSlot ────────────────────────────────────────────────────────
// Fades between a video thumbnail (with play button) and an alt photo image.

function VideoCarouselSlot({
  thumbnail,
  name,
  altImage,
  col,
  ...
  const [active, setActive] = useState(0) // 0 = video thumb, 1 = alt photo
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null)
...
// 6-column × 2-row mosaic — 13 photos across 7 slots
// Row 1: ballroom wides (1-4) · step-repeat duo (4-6) · portrait pair (6-7)
// Row 2: portrait+stage (1-2) · stage/candlelit (2-4) · DJ+curtain (4-6) · band (6-7)
...
// 2025 — same slot structure, all placeholders until photos arrive
const GALA_25_MOSAIC: { pair: [null, null]; delay: number; col: string; row: string }[] = [
  { pair: [null, null], delay: 0, col: '1 / 4', row: '1 / 2' },
  { pair: [null, null], delay: 0, col: '4 / 6', row: '1 / 2' },
  { pair: [null, null], delay: 0, col: '6 / 7', row: '1 / 2' },
  { pair: [null, null], delay: 0, col: '1 / 2', row: '2 / 3' },
  { pair: [null, null], delay: 0, col: '2 / 4', row: '2 / 3' },
  { pair: [null, null], delay: 0, col: '4 / 6', row: '2 / 3' },
  { pair: [null, null], delay: 0, col: '6 / 7', row: '2 / 3' },
]

// ─── GalaGallery ─────────────────────────────────────────────────────────────

function GalaGallery() {
  return (
    <section id="past-galas" className="relative pb-24 overflow-hidden" style={{ paddingTop: 40 }}>
      <img src={galaBg} alt="" aria-hidden="true"
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        ...
        {/* 3-row mosaic — mirrors ZT Athletics layout exactly */}
        <div className="grid gap-2 mb-10"
          style={{ gridTemplateColumns: 'repeat(6, 1fr)', gridTemplateRows: 'repeat(3, 240px)' }}>
          {/* Row 1: wide(3) · video(2) · narrow(1) */}
          ...
          {/* Row 2: narrow(1) · video(2) · narrow(1) · medium(2) */}
          ...
          {/* Row 3: medium(2) · narrow(1) · video(2) · narrow(1) */}
...
export default function App() {
  ...
  return (
    <div
      ...
      style={{ background: OBSIDIAN, color: CREAM, fontFamily: "'Brother 1816', 'Helvetica Neue', sans-serif" }}
    >
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/* NAV                                                                */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      ...
      >
        ...
        <div className="absolute inset-0">
          ...
          />
          {/* Bottom gradient only — darkens under event info, fades out softly */}
          <div
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-6 pt-48 pb-20">
          ...
          <div className="flex flex-col sm:flex-row items-stretch gap-8 sm:gap-0 mb-14">
            {[
              ...
              >
                <p
                  className="text-[8px] tracking-[0.5em] uppercase mb-3"
      ...
      <section id="performers" style={{ background: DEEP, position: 'relative', overflow: 'hidden' }} className="pb-24 pt-5 px-0">
        <div className="max-w-7xl mx-auto px-6 lg:px-12" style={{ position: 'relative', zIndex: 1 }}>
          ...
          {/* ── LeAnn Rimes — featured headliner ── */}
          <div className="mb-16">
        ...
        {/* Hosts background overlay — wave pattern at bottom of section */}
        <div
          aria-hidden="true"
          style={{
            position: 'absolute',
      ...
      <section id="foundation" style={{ background: OBSIDIAN }} className="relative py-24 overflow-hidden">
        {/* ZT Athletics icon — left background watermark */}
        <img
          src={ztaWordMarkIcon}
          aria-hidden="true"
        ...
        />
        {/* Right-side image — sits behind content */}
        <div className="absolute inset-y-0 right-0 pointer-events-none" style={{ width: '52%' }}>
          ...
          <div className="absolute inset-0" style={{
            background: 'linear-gradient(to right, rgba(43,27,12,1) 0%, rgba(43,27,12,0.80) 18%, rgba(43,27,12,0.20) 55%, transparent 100%)',
          }} />
          {/* Top + bottom fades */}
          <div className="absolute inset-x-0 top-0" style={{
            height: '35%',
            background: 'linear-gradient(to bottom, rgba(43,27,12,0.85) 0%, transparent 100%)',
          }} />
          <div className="absolute inset-x-0 bottom-0" style={{
            height: '35%',
            background: 'linear-gradient(to top, rgba(43,27,12,0.85) 0%, transparent 100%)',
          }} />
        </div>

        {/* Bottom-left 45° gold gradient — ~25% coverage */}
        <div className="absolute bottom-0 left-0 pointer-events-none" style={{
          width: '45%',
          height: '45%',
          background: 'radial-gradient(ellipse at 0% 100%, rgba(198,162,97,0.55) 0%, rgba(198,162,97,0.30) 30%, rgba(198,162,97,0.10) 55%, transparent 72%)',
        }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start mb-20">
            <div>
              ...
              <img
                src={ztaWordMark}
                alt="ZT Athletics Foundation"
                className="mb-8"
                style={{ width: 320, mixBlendMode: 'screen' }}
              />
            ...
            {/* Pull quote */}
            <div
              className="lg:self-center p-10 lg:p-14"
              style={{
                background: 'rgba(125,91,36,0.05)',
                borderTop: `1px solid rgba(198,162,97,0.18)`,
                borderRight: `1px solid rgba(198,162,97,0.18)`,
                borderBottom: `1px solid rgba(198,162,97,0.18)`,
          ...
          {/* Masonry Mosaic Collage — 3 rows, 6-column grid
              Row 1: photo(3) · video(2) · photo(1)
              Row 2: photo(1) · video(2) · photo(1) · video(2)
              Row 3: photo(2) · photo(1) · video(2) · photo(1)          */}
          <div
      ...
      >
        ...
        <img
          ...
          className="absolute inset-0 w-full h-full object-cover object-center"
        />
        {/* Scrim — keeps cards readable without killing the texture */}
        <div
          className="absolute inset-0"
          style={{ background: 'rgba(27,14,4,0.62)' }}
        />
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            <h2 className="font-quiche text-4xl lg:text-5xl mb-4" style={{ color: CREAM }}>
              Sponsorship
            </h2>
          ...
          {/* 6-card single row — breaks out to full viewport width */}
          <div
            className="mb-20"
            style={{
              position: 'relative',
          ...
          >
            {TIERS.map((tier) => {
              ...
              const btnBorder   = tier.badgeGold ? LUX_GOLD : 'rgba(255,255,255,0.28)'
              const btnColor    = tier.badgeGold ? LUX_GOLD : 'rgba(255,255,255,0.7)'
              return (
                <div
                  key={tier.name}
                ...
                >
                  {/* Top accent line */}
                  <div
                    className="pricing-card__accent absolute top-0 left-0 right-0 h-[2px]"
                  />

                  {/* Badge */}
                  {tier.badge && (
                    <p
                      className="text-[9px] tracking-[0.4em] uppercase mb-5 font-medium"
                      style={{ color: tier.badgeGold ? LUX_GOLD : LUX_MUTED }}
                    >
                      {tier.badge}
                    </p>
                  )}
                  ...
                  {/* Title */}
                  <h3
                    className="font-quiche text-lg mb-3 uppercase tracking-wide"
                    style={{ color: '#ffffff' }}
                  >
                    {tier.name}
                  </h3>
                  ...
                  {/* Price */}
                  <p
                    className="font-quiche text-4xl mb-6"
                  ...
                  {/* Perks — flex-grow pushes button to bottom */}
                  <ul className="space-y-3 flex-grow">
                    {tier.perks.map((perk) => (
                      ...
                      >
                        <span
                          className="flex-shrink-0 mt-[3px] text-[8px]"
                          style={{ color: LUX_GOLD }}
                        >
                          ◆
                        </span>
                        {perk}
      ...
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/* PARTNER WALL — full-bleed light section                           */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section className="relative w-full py-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
          ...
          <div className="text-center mb-14">
            <h2
              className="font-quiche text-4xl lg:text-5xl mb-3"
              style={{ color: '#4D310A' }}
            >
              Thank You
            </h2>
            <p
              ...
              style={{ color: 'rgba(77,49,10,0.45)', fontWeight: 900 }}
            >
          ...
          <div className="space-y-12">
            {SPONSOR_TIERS.map(({ tier, logoHeight, entries }) => (
              <div key={tier}>
                ...
                {/* Logos — multiply blend dissolves white into cream */}
